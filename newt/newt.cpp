...
#define M_T_EXIT 103
#define M_B_EXIT 104
#define M_EXIT_MAX 110
#define M_EXIT_MIN 101
...
public:
    //NPC(fnt& NPC_UID) {};
    NPC(int ntp, fnt& uid, wstring& n, wifstream& wis) :type(ntp), uuid(uid), name(n), dg(wis, n) {};//临时构造函数
    ~NPC() {};
    void talk() {
        dg.ViewGw();
        int task = 9;
...
private:
    ...
    vector<NPC> npcs;
public:
    maps(wifstream& wis) :siz{ 0 }, Mp{ 0,0,0,0 }, pp{0, 0, UP}{ {
            pp.x_pos = 12;
            pp.y_pos = 12;
            pp.dir = UP;
            siz[12][0] = M_L_EXIT;
            ...
            fnt anid = 1;
            fnt bnid = 2;
            NPC bq((int)Task, anid, bn, wis);
            NPC aq((int)Task, anid, an, wis);
            npcs.push_back(bq);
            npcs.push_back(aq);
            wchar_t tmpsa[10]{L"风之峡谷\0"};
            for (int i = 0; i < 10; i++)
                Mp.Name[M_L_EXIT - M_L_EXIT][i] = tmpsa[i];
            //////////以上为debug定义/////////////
            for (int n = 0; n < 24; n++)
                for (int j = 0; j < 24; j++)
                {
                    ...
                    else if (M_EXIT_MAX < siz[n][j] and 120 >= siz[n][j])
                    {
                        oupic[n][j] = L'◆';
                    }
                    else if (M_EXIT_MIN <= siz[n][j] and siz[n][j] <= M_EXIT_MAX)
                    {
                        oupic[n][j] = L'Π';
                    }
                }
    ...
    bool if_invaild(sint x, sint y) {
        if (x < 0 or x >= 24 or y < 0 or y >= 24)
            return true;
        else if (siz[y][x] >= M_EXIT_MIN and siz[y][x] <= M_EXIT_MAX)
            return true;
        else if (siz[y][x] > M_EXIT_MAX and siz[y][x] <= 120)
            return true;
        return false;
    }
    inline void reaction(int forward_item) {
        if (M_EXIT_MIN <= forward_item and M_EXIT_MAX >= forward_item)
            wcout << L"按E前往" << Mp.Name[forward_item - M_EXIT_MIN] << endl;
        else if (M_EXIT_MAX < forward_item and 120 >= forward_item)
